...
class Post {	
	...
	public function getPostsListing(){		
		...
		if($_SESSION['is_admin'] == 0) {
			$whereQuery = "WHERE experiences.userid ='".$_SESSION['userid']."'";
		}	
		
		$sqlQuery = "
			SELECT experiences.story_id, experiences.title, experiences.category, users.FirstName, users.last_name, experiences.date_added
			FROM ".$this->postTable." experiences
			LEFT JOIN ".$this->userTable." users ON users.id = experiences.userid
			$whereQuery";
		
		if(!empty($_POST["search"]["value"])){
			$sqlQuery .= ' title LIKE "%'.$_POST["search"]["value"].'%" ';
			$sqlQuery .= ' OR location LIKE "%'.$_POST["search"]["value"].'%" ';
			$sqlQuery .= ' OR Category LIKE "%'.$_POST["search"]["value"].'%" ';
			$sqlQuery .= ' OR date_added LIKE "%'.$_POST["search"]["value"].'%" ';			
		}
