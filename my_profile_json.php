    ...
    try {
        ...
        */
        ...
        if (isset($_GET["keyword"])) {
            ...
            $query = $query . "  WHERE userid = '$username' AND  (experiences.title LIKE '%" . $keyword . "%' OR experiences.location like '%" . $keyword . "%' OR experiences.category like '%" . $keyword . "%' OR userid like '%" . $keyword . "%'  OR date_added like '%" . $keyword . "%' )"; //append filter to SQL query using OR to filter by more

            $query = $query . " ORDER BY story_id DESC";
        } else {
            $query = "Select * FROM experiences  WHERE userid = '$username' ";
        }
