DROP DATABASE IF EXISTS `cm007`;
CREATE DATABASE `cm007` /*!40100 DEFAULT CHARACTER SET latin1 */;
USE `cm007`;
...
ON DUPLICATE KEY UPDATE `story_id` = VALUES(`story_id`), `title` = VALUES(`title`), `location` = VALUES(`location`), `description` = VALUES(`description`), `category` = VALUES(`category`), `image` = VALUES(`image`), `userid` = VALUES(`userid`), `date_added` = VALUES(`date_added`);
...
ON DUPLICATE KEY UPDATE `id` = VALUES(`id`), `FirstName` = VALUES(`FirstName`), `LastName` = VALUES(`LastName`), `phone` = VALUES(`phone`), `email` = VALUES(`email`), `hashed_password` = VALUES(`hashed_password`), `is_admin` = VALUES(`is_admin`);
